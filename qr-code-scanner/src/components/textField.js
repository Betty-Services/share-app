(() => ({
  ...
  jsx: (() => {
    ...
    const { getModel, getProperty } = B;

    if (!isDev && customModelAttributeObj) {
      if (customModelAttributeObj.id) {
        const customAttributeObj = getCustomModelAttribute(
          customModelAttributeObj.id,
        );
        ...
        if (customAttributeObj && customAttributeObj.propertyId) {
          ...
          if (customPropertyObj.modelId) {
            ...
            const propertyName = `${camelToSnakeCase(
              customPropertyModelObj.name,
            )}_${customPropertyObj.name}`;
    ...
    const TextFieldCmp = (
      ...
      >
        ...
        <InputCmp
          ...
          data-exam-component={examDataComponentAttribute}
        />
        {helper && (
